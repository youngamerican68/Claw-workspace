  ...
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      if (status.data.status === 'SUCCEEDED') {
        ...
        const { analyzeTweets } = require('./analyzer.js');
        await analyzeTweets(path.join(config.outputDir, `tweets-${timestamp}.json`));
        
        // Flag for AI to do deep analysis
        fs.writeFileSync('/root/.openclaw/workspace/twitter-scout/NEW_DATA_READY', timestamp);
        console.log('New data flagged for deep analysis');
